@testset "CoverEnumerator" begin # FIXME use weights
    @testset "[:a] [:b] [:c] [:a :b :c], mask=[:a :b]" begin
        ...
        cover_etor1 = CoverEnumerator(sm_ab, CoverParams(a=0.5, b=0.1, sel_prob=1.0))
        ...
        cover_etor2 = CoverEnumerator(sm_ab, CoverParams(a=0.5, b=0.1, sel_prob=0.01))
        ...
        cover_etor3 = CoverEnumerator(sm_ab, CoverParams(a=0.1, b=0.1, sel_prob=1.0))
    ...
    @testset "[:a] [:b] [:c] [:a :b :c] :d :e, mask=[:a :b]" begin
        ...
        # higher prior probability to select sets, lower probability to miss active element, so select a and b, then abc
        cover_etor = CoverEnumerator(sm_ab, CoverParams(a=0.1, b=0.1, sel_prob=1.0))
