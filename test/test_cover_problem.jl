@testset "CoverProblem" begin
    ...
    @testset "empty" begin
        ...
        problem = CoverProblem(msm, CoverParams(0.1, 0.2, 0.5))
    ...
    @testset "[:a]" begin # FIXME take element detection probability into account
        # empty mask problem
        empty_problem = CoverProblem(mask(SetMosaic([Set([:a])]), Set{Symbol}()), CoverParams(0.5, 0.5))
        ...
        @test dis_res.weights ≈ [0.0] atol=1E-8
        ...
        @test_broken en_res.weights ≈ [1.0] atol=1E-8
    end
    ...
    @testset "[:a :b] [:c :d] [:a :b :c :d], mask=[:a :b]" begin # FIXME take weights into account
        ...
        problem_ab_lowp = CoverProblem(sm_ab, CoverParams(0.1, 0.1))
        @test_skip nsets(problem_ab_lowp) == 2

        res_ab_lowp = optimize(problem_ab_lowp)
        @test_skip res_ab_lowp.weights ≈ [0.0, 0.0] atol=1E-4

        problem_ab = CoverProblem(sm_ab, CoverParams(0.1, 0.1))
        res_ab = optimize(problem_ab)
        @show res_ab.weights
        @test_broken res_ab.weights ≈ [1.0, 0.0] atol=1E-4
    end
    ...
    @testset "[:a] [:b] [::c] [:a :b :c], mask=[:a :b]" begin # FIXME take weights into account
        ...
        problem_ab_lowp = CoverProblem(sm_ab, CoverParams(0.4, 0.1))
        ...
        problem_ab = CoverProblem(sm_ab, CoverParams(0.1, 0.1))
        @test_broken score(problem_ab, [1.0, 1.0, 0.0]) < score(problem_ab, [0.0, 0.0, 1.0])
        res_ab = optimize(problem_ab)
        @test_broken res_ab.weights ≈ [1.0, 1.0, 0.0] atol=1E-4
    end
end
