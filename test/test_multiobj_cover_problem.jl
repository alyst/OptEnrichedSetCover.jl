@testset "MultiobjectiveCoverProblem" begin
    ...
    @testset "[a]" begin # FIXME take element detection probability into account
        ...
        dis_problem = MultiobjectiveCoverProblem(mask(SetMosaic([Set([:a])], Set([:a])), [Set{Symbol}([:a])], min_nmasked=1))
        @test nvars(dis_problem) == 1
        dis_res = optimize(dis_problem)
        @test dis_res.weights == zeros(Float64, 1)

        # enabled because there is :a and :b
        en_problem = MultiobjectiveCoverProblem(mask(SetMosaic([Set([:a])], Set([:a, :b])),
    ...
    @testset "[a b] [c d] [a b c d], mask=[a b]" begin # FIXME take weights into account
        ...
        problem_no_penalty = MultiobjectiveCoverProblem(sm_ab, CoverParams(setXset_factor=0.0, sel_prob=1.0))
    ...
    @testset "[a b] [b c] [a b c], mask=[b]" begin # FIXME take weights into account
        ...
        problem_ignore_overlap = MultiobjectiveCoverProblem(sm_b, CoverParams(setXset_factor=10.0, sel_prob=1E-25))
        ...
        problem_b = MultiobjectiveCoverProblem(sm_b)
